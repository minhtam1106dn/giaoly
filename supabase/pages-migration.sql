...
begin
  ...
  if p_data ? 'pages' then
    if jsonb_typeof(p_data->'pages') is distinct from 'object' then raise exception 'Invalid presentation pages'; end if;
    for engine in select unnest(array['opening','closing']) loop
      page:=p_data->'pages'->engine;
      if jsonb_typeof(page) is distinct from 'object' or jsonb_typeof(page->'enabled') is distinct from 'boolean' or jsonb_typeof(page->'title') is distinct from 'string' or length(page->>'title')>160 or ((page->>'enabled')::boolean and length(trim(page->>'title'))=0) or jsonb_typeof(page->'body') is distinct from 'string' or length(page->>'body')>4000 then raise exception 'Invalid presentation page'; end if;
    end loop;
  end if;
  ...
  if p_data ? 'pages' then clean_public:=clean_public||jsonb_build_object('pages',p_data->'pages'); end if;
