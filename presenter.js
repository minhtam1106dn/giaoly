...
import {loadPublic,demo} from './api.js';
...
async function load(){try{data=await loadPublic();questions=data.questions.filter(q=>q.enabled);$('#session-title').textContent=data.title;$('#empty').hidden=questions.length>0;$('#presentation').hidden=!questions.length;$('#jump').innerHTML=questions.map((_,i)=>`<option value="${i}">${i+1}</option>`).join('');if(questions.length)show(0);}catch(e){$('#load-error').hidden=false;$('#load-error').className='notice';$('#load-error').textContent=e.message;}}
